 ...
 * @version $Revision: 1 $
 */
public class AdaptiveRandomForestBagFeatures extends AbstractClassifier implements MultiClassClassifier,
                                                                        CapabilitiesHandler {
    ...
    public void resetLearningImpl() {
        ...
        if(numberOfJobs != AdaptiveRandomForestBagFeatures.SINGLE_THREAD && numberOfJobs != 1)
    ...
    public void trainOnInstanceImpl(Instance instance) {
        ...
        for (int i = 0 ; i < this.ensemble.length ; i++) {
            ...
            //JEAN
            //System.out.println("Ensemble tree attributes: " + this.ensemble[i].classifier.li);
        }
        if(this.executor != null) {
            try {
...
//        BasicClassificationPerformanceEvaluator classificationEvaluator = (BasicClassificationPerformanceEvaluator) getPreparedClassOption(this.evaluatorOption);
        ...
        switch(this.mFeaturesModeOption.getChosenIndex()) {
            case AdaptiveRandomForestBagFeatures.FEATURES_SQRT:
                this.subspaceSize = (int) Math.round(Math.sqrt(n)) + 1;
                break;
            case AdaptiveRandomForestBagFeatures.FEATURES_SQRT_INV:
                this.subspaceSize = n - (int) Math.round(Math.sqrt(n) + 1);
                break;
            case AdaptiveRandomForestBagFeatures.FEATURES_PERCENT:
    ...
    public ImmutableCapabilities defineImmutableCapabilities() {
        if (this.getClass() == AdaptiveRandomForestBagFeatures.class)
