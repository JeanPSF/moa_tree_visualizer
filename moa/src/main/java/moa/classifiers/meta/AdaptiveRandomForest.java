...
//        ArrayList<double []> allVotes = new ArrayList<>();
        for(int i = 0 ; i < this.ensemble.length ; ++i) {
            double votesExpert[] = this.ensemble[i].getVotesForInstance(testInstance);
            // Utils.normalize(votesExpert);
//            allVotes.add(votesExpert);
            DoubleVector vote = new DoubleVector(votesExpert);
            if (vote.sumOfValues() > 0) {
//            if (vote.sumOfValues() > 0.0001) {
                vote.normalize();
                double acc = this.ensemble[i].evaluator.getPerformanceMeasurements()[1].getValue();
                if(! this.disableWeightedVote.isSet() && acc > 0.0) {
                    for(int v = 0 ; v < vote.numValues() ; ++v) {
                        vote.setValue(v, vote.getValue(v) * (acc > 0 ? acc : 1));
                    }
                }
...
//        if(!Double.isFinite(Utils.sum(combinedVote.getArrayRef()))){
//            int debug = 1;
//        }
        // System.out.println(combinedVote.getArrayRef());
//        System.out.println(Arrays.toString(combinedVote.getArrayRef()));
...
//        BasicClassificationPerformanceEvaluator classificationEvaluator = (BasicClassificationPerformanceEvaluator) getPreparedClassOption(this.evaluatorOption);
    ...
    public double[] getFeatureScores() {
        ...
        for(int i = 0; i < numTrees; i++) {
            double[] treeScores = this.ensemble[i].getFeatureScores();
            if(treeScores[1] > 0.0){
                treeScores = this.ensemble[i].getFeatureScores();
            }
            fScores = Utils.aggregate(fScores, treeScores);
        }
