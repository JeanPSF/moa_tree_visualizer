...
/*    public String[] getTreeAtributes() {
        HashMap<String, Integer> availableAttrs = ;
    }*/
    ...
    public static class RandomLearningNode extends ActiveLearningNode {
        ...
        protected String[] listActiveAttributes;
        ...
        public void learnFromInstance(Instance inst, HoeffdingTree ht) {            
            ...
            if (this.listAttributes == null) {
                ...
                int validAttCounter = 0;
                HashMap<String, Integer> map = inst.getAttributesList();

                if(ht.fixedAttrOption.getValue() != null) {
                	this.listActiveAttributes = ht.fixedAttrOption.getValue().split("\\;");
                    for(int x = 0; x < this.listActiveAttributes.length; x++) {
                    	if(map.containsKey(listActiveAttributes[x])){
                    		this.listAttributes[validAttCounter] = map.get(listActiveAttributes[x]);
                    		validAttCounter++;
                    	}
                    }
                }
                System.out.println("Atributes available: " + map);
                for (int j = validAttCounter; j < this.numAttributes; j++) {
                ...
                System.out.print("Attributes selected: ");
                for(int i = 0; i < this.listAttributes.length; i++){
                    System.out.print(this.listAttributes[i] + ", ");
                }
            }
